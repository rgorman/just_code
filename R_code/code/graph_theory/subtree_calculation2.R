...
for (j in 1:length(sentence.list)) {
 ...
 #############################################
  ...
  for (k in 1:(length(sentence.list[[j]])-1)) {
    ...
    ##################
    # The following code block checks each vertex in a given sentence for projectivity.
    
    
    if (length(projection.v) > 0)  {
      projection.span.v <- min(projection.v):max(projection.v)
      projection.gaps.v <- setdiff(projection.span.v, projection.v)  
      # proj.collector <- append(proj.collector, projection.gaps.v)
    }
    
   
    if (length(projection.gaps.v) < 1 ) {
      projectivity.attr <- "Projective"
      
      
    } else {
      
      relat.counter.v <- 0
      punctuation.v <- c("AuxX", "AuxK", "AuxG")
      
      for (m in 1:length(projection.gaps.v)) {
        
        relat.v <- sentence.list[[j]][projection.gaps.v[m]]
        relat.v <- unlist(relat.v)
        relat.v <- relat.v["word.relation"]
        
        if (length(setdiff(punctuation.v, relat.v)) < 3) {
          
         # relat.counter.v <- relat.counter.v + 0
          
        } else {
          
          relat.counter.v <- relat.counter.v + 1
          
        }
        
      }
      
      if (relat.counter.v > 0) {
        
        projectivity.attr <- "NonProjective"  
...
saveXML(subtree.xml, file = "./test_output/test2_Feb-11.xml")
...
for (k in 1:(length(sentence.list[[j]])-1)) {
  ...
  if (length(projection.v) > 0)  {
    projection.span.v <- min(projection.v):max(projection.v)
    projection.gaps.v <- setdiff(projection.v, projection.span.v)  
    proj.collector <- append(proj.collector, projection.gaps.v)
  }
...
y <- projection.v
setdiff(x,y)
setdiff(projection.v, projection.span.v)
setdiff(projection.span.v, projection.v)
