...
for (i in 1: length(files.v))  {
  ...
  file.DepDist.v <- NULL # sentence mean dep dist ???
  ...
  for (j in 1:length(sentence.list)) {
    ...
    rel.holder.v <- NULL # holder vectors made at the sentence level (loop j) to pass values to new word elements.
                         # These vectors hold values for neighborhood subtrees of each word in sentence.
    
    pos.holder.v <- NULL  # Holds part of speech values for each word in sentence.
    rel.pos.holder.v <- NULL # Holds combination of relation and part of speech values for each word in sentence.
    ...
    for (k in 1:(length(sentence.list[[j]])-1)) {
      b <- unlist(sentence.list[[j]][[k]]) # vector of attributes for each word element.
      if ("insertion_id" %in% names(b)) { # selects word identified as an ellipsis.
        
        head.ellipsis.v <- append(head.ellipsis.v, TRUE) # TRUE value in vector for each ellipsis in sentence. 
        
      } else { # selects words that are not ellipses.
        
        head.ellipsis.v <- append(head.ellipsis.v, FALSE) # FALSE value for non-ellipses.
        
      }
      
      
    } # end of loop k to identify ellipses.
    ...
    for (k in 1:(length(sentence.list[[j]])-1))  {
      ...
      if (heads.v[k] > 0)  { # selects words whose head attribute value is not 0. Values of 1 or greater indicate that word k
                             # is a dependency and thus might depend on an ellipsis.
        
        if (head.ellipsis.v[heads.v[k]] == FALSE) { # selects words for which head.ellipsis.v indicates its head is not an
                                                    # ellipsis.
          
          depend.on.ellip.v <- append(depend.on.ellip.v, FALSE) # FALSE added to appropriate vector.
      ...
      } else { # selects word k with head value of 0.  This word is not a dependency and cannot be the child of an ellipsis. 
    ...
    } # end of loop k to identify nodes dependent on ellipses.
    ...
    holder.node.DepDist.v <- NULL # Vector to hold DD for sentence. ???
